...
def main(cfg: DictConfig):
    ...
    print("This app is for classes and color file generation. It is for replica dataset")
    
    # Generate output path
    classes_info_path = os.path.join(cfg.output_path, str(
        cfg.dataset_name) + "_" + str(cfg.scene_id), "classes_info")
    ...
    class_id_colors = {int(class_id): unique_colors[i].tolist() for i, class_id in enumerate(unique_class_ids)}
    
    # Get Dict: names --> colors
    names_colors = {class_id_names[int(class_id)] : color for class_id, color in class_id_colors.items()}    
    
    # Save names .txt
    # get path
    names_path = os.path.join(classes_info_path, f"{cfg.dataset_name}_{cfg.scene_id}_names.txt")
    print(names_path)
    ...
    # get path
    names_colors_path = os.path.join(classes_info_path, f"{cfg.dataset_name}_{cfg.scene_id}_names_colors.json")
    print(names_colors_path)
    with open(names_colors_path, "w") as f:
        json.dump(names_colors, f)
        
    # Save class idx --> colors .json
    class_id_colors_path = os.path.join(classes_info_path, f"{cfg.dataset_name}_{cfg.scene_id}_id_colors.json")
    print(class_id_colors_path)
    with open(class_id_colors_path, "w") as f:
        json.dump(class_id_colors, f)
    
    # Save class idx --> names .json
    class_id_names_path = os.path.join(classes_info_path, f"{cfg.dataset_name}_{cfg.scene_id}_id_names.json")
