...
def main(cfg: DictConfig):
    ...
    if cfg.yolo.use_given_classes:
        ...
        class_id_colors_path = os.path.join(
            cfg.output_path, f"{cfg.dataset_name}_{cfg.scene_id}", "classes_info", f"{cfg.dataset_name}_{cfg.scene_id}_id_colors.json")
    ...
    if cfg.yolo.use_given_classes:
        ...
        class_id_names_path = os.path.join(
            cfg.output_path, f"{cfg.dataset_name}_{cfg.scene_id}", "classes_info", f"{cfg.dataset_name}_{cfg.scene_id}_id_names.json")
    ...
    if os.path.exists(cfg.map_dir):
        ...
        load_dir = os.path.join(cfg.output_path, f"{cfg.dataset_name}_{cfg.scene_id}", "map")
    ...
    def query_callback(vis):
        ...
        for i, (cos_val, idx) in enumerate(zip(top_k_cos_sim.tolist(), top_k_idx.tolist())):
            print(f"{i+1}. No. {idx} {class_id_names[obj_map[idx].class_id]}: {cos_val:.3f}")
